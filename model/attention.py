...
class hDS_RNN_wtos(nn.Module):
    def __init__(self, input_size, args):
        '''
        :param input_size: int, input size
        :param args: global variables
        '''
        super(hDS_RNN_wtos, self).__init__()
        self.model_name = 'hDS_RNN'
        self.details = 'None'
        ...
        self.mid_layer = nn.LSTM(input_size=self.n_hid,
                                 ...
                                 batch_first=True,
                                 dropout=0.5)
        self.no_spatial_layer = nn.LSTM(input_size=self.n_inp,
                                        hidden_size=self.n_hid,
                                        num_layers=1,
                                        batch_first=True,
                                        dropout=0.5)
    ...
    def forward(self, input_p_q, label_p):
        batch_size = input_p_q.shape[0]
        # get the inputs for the encoder
        inputs = input_p_q[:, :self.T_enco, :]  # 64*60*x
        #
        # # # Spatial attention
        # h = torch.zeros(batch_size, self.n_hid, device=device)
        # c = torch.zeros(batch_size, self.n_hid, device=device)
        ...
        # mid layer
        mid_output, hh = self.no_spatial_layer(inputs)
        mid_output, hh = self.mid_layer(mid_output)  # [64*60*64]
...
class hDS_RNN_wsot(nn.Module):
    def __init__(self, input_size, args):
        '''
        :param input_size: int, input size
        :param args: global variables
        '''
        super(hDS_RNN_wsot, self).__init__()
        self.model_name = 'hDS_RNN'
        self.details = 'None'
        ...
        self.regressor = nn.Linear(self.n_hid_deco, 1)
        self.Tanh = nn.Tanh()
        self.regressor2 = nn.Linear(60, 4)
    ...
    def forward(self, input_p_q, label_p):
        ...
        #     # self.temporal_att_score_list += [t_att_score.transpose(1, 2).softmax(dim=2)]
        #     # decoder_in = torch.bmm(t_att_score.transpose(1, 2), mid_output)
        #     decoder_in = mid_output
        #     hi, ci = self.decoder(decoder_in.squeeze(), (hi, ci))
        #     decode_output += [hi.unsqueeze(1)]

        # output regressor
        # out = torch.cat(decode_output, dim=1)
        out = self.regressor(mid_output)
        out = self.regressor2(out.squeeze())
        # out = out[:, -self.T_deco:, :].squeeze()
        return out
